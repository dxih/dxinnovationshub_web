import { 
  Box, Container, Typography, TextField, Radio, RadioGroup, 
  FormControlLabel, FormControl, FormLabel, Stack, Button, 
  Divider, Paper, alpha 
} from '@mui/material';
...
const GetStartedPage = () => {
  const selectionRowStyle = {
    m: 0, mb: 1.5, p: 1.5, borderRadius: '12px', border: '1px solid #e2e8f0',
    transition: '0.2s all ease', width: '100%',
    '&:hover': { bgcolor: '#f8fafc' },
    '&:has(input:checked)': { borderColor: 'primary.main', bgcolor: alpha('#FFC843', 0.08) }
  };

  return (
    <Box sx={{ bgcolor: '#fcfcfd', minHeight: '100vh', pt: { xs: 12, md: 15 }, pb: 10 }}>
      <Container maxWidth="md">
        ...
        <Paper elevation={0} sx={{ p: { xs: 3, md: 8 }, borderRadius: '32px', border: '1px solid #f1f5f9' }}>
          <form action="https://formspree.io/f/xpqldvaj" method="POST">
            
            {/* Header Section */}
            <Typography variant="overline" sx={{ color: 'primary.main', fontWeight: 800, letterSpacing: 1.5, fontSize: '0.75rem' }}>
              DX Innovations Hub
            </Typography>
            <Typography variant="h4" sx={{ fontWeight: 700, mb: 1, color: 'secondary.main', fontSize: { xs: '1.8rem', md: '2.5rem' } }}>
              Get Started
            </Typography>
            <Typography variant="body1" sx={{ mb: 6, color: 'text.secondary', fontWeight: 500, maxWidth: '600px', lineHeight: 1.6 }}>
              Turn ideas, challenges, or ambitions into real digital outcomes. Share a bit about what you need, and we’ll guide the next move.
            </Typography>
